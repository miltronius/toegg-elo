...
- **Win/loss is read from `elo_history.won`, never from the sign of `elo_change`.** Margin scoring makes a winner's delta non-negative again, but not *positive* - it rounds to 0 across a wide enough gap - and the 2026-08 → 2026-09 rows rated by the old model still hold winners with negative deltas. `frontend/src/lib/eloHistory.ts` (`didWin`/`didLose`) is the only place that decision is made; it falls back to the old sign rule for rows where `won` is null (inactivity penalties, and anything written between the migration and the function deploy). Achievements were always safe here because they read `matches.winning_team`.
