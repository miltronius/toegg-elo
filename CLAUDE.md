...
- **A recorded match is a series of N games**, not a single result. `ΔR = K · Σ_games(S − E)` - the FIDE model - so 2:0 and 2:1 differ automatically without inventing a fractional score. E is taken once from the pre-series ratings, making the result independent of the order the games were played in. A consequence worth knowing before it surprises someone: because the `n·E` term grows with series length, a heavy favourite can win a long series and *still* lose rating (a 3:2 at E = 0.75 is a net loss). That is the margin sensitivity working, and `rateSeries` is where a floor would go if the league ever rejects it - at the cost of the zero-sum property.
...
- **The four deltas sum to exactly zero.** This holds because each player's E is the *average of their pairwise expectations* (linear, and pairwise expectations are complementary), so the four E values always total 2 - the score on offer per game. Two easy ways to break it: averaging the opponents' **ratings** instead of their expectations (the logistic isn't linear), or blending only your own team and leaving the opponents raw. Rounding also happens once across all four (`roundPreservingSum`), or each delta rounding on its own would leak up to 2 points per match.
- **`k_factor` is per game, not per recorded match.** Season 2026-08 runs at K=48, which is not the cut from the old 64 it looks like: under the old one-event-per-match model an even session moved a flat ±32, whereas 48 per game gives 48 for an even 2-0 and 24 for an even 2-1. Read the season K with that in mind before comparing it to a pre-series season.
