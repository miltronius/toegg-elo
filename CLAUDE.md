...
- **Partner weighting** (`seasons.partner_weight`, default 1/3): each player's rating is pulled that far towards their partner's *before* expectations are computed. `w = 0` ignores the partner (the pre-2026-08 behaviour, where a 1900+1100 pair beating two 1500s gave `+2.91` and `+29.09` for one identical event); `w = 0.5` collapses both onto the team average, giving both partners the same delta; `w = 1/3` sits between. Per season like `k_factor`, so it is re-tunable at a season boundary without a deploy.
