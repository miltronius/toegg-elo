...
function setup(props: Partial<React.ComponentProps<typeof RelationshipGraph>> = {}) {
...
describe('RelationshipGraph', () => {
  ...
  it('shows an empty state when nothing has been played', () => {
    ...
    expect(screen.getByText(/no matches in this season yet/i)).toBeInTheDocument();
...
describe('RelationshipGraph — friends/foes toggle', () => {
  it('starts on friends', () => {
    ...
    expect(screen.getByRole('button', { name: 'Foes' })).toHaveAttribute('aria-pressed', 'false');
  });
  ...
  it('switches to foes on click', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Foes' })).toHaveAttribute('aria-pressed', 'true');
...
describe('RelationshipGraph — 3D', () => {
  ...
  it('resizes labels from the slider', () => {
    ...
    fireEvent.change(screen.getByLabelText(/label size/i), { target: { value: '24' } });
...
describe('RelationshipGraph — interaction', () => {
  ...
  it('thins the edges but keeps the nodes when the budget is lowered', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/top partners/i), { target: { value: '1' } });
