...
export function MatchHistory({
  ...
  const seasonStatsMap = new Map(
    playerSeasonStats.map((s) => [
      `${s.player_id}-${s.season_id}`,
      s.elo_at_start,
    ]),
  );
  const getSeasonStartAlltimeElo = (
    playerId: string,
    seasonId: string,
  ): number | null => seasonStatsMap.get(`${playerId}-${seasonId}`) ?? null;
        ...
        <div className="flex flex-col gap-6">
          {visibleMatches.map((match, index) => {
            ...
            const renderPlayerEloDetails = (
              ...
              return (
                <div className="flex items-center gap-3 text-[0.9rem] px-2 py-1.5 rounded bg-white/40">
                  ...
                  <span className="text-text-light text-[0.85rem]">
                    {eloBefore}
                  </span>
                  <span className="text-text-light font-light">→</span>
                  <span
                    className={clsx(
                      "font-semibold min-w-11.25",
                  ...
                  <span
                    className={clsx(
                      "text-[0.8rem] px-2 py-1 rounded font-medium",
                      change > 0
            ...
            const season = match.season_id
              ? seasonMap.get(match.season_id)
              : undefined;

            return (
              <div
                key={match.id}
                className="bg-bg border border-border rounded-lg p-6 transition-all hover:shadow-md"
              >
                <div className="flex items-center gap-6 mb-4 max-sm:flex-col max-sm:gap-4">
                  <div
                    className={clsx(
                      "flex-1 p-4 rounded-md border-2",
                      teamAWon
                        ? "bg-success-light border-success"
                        : "bg-error-light border-error opacity-70",
                    )}
                  >
                    <div className="font-semibold mb-3 text-text">
                      {t("matchHistory.teamA")} {teamAWon && "✓"}
                    </div>
                  ...
                  <div className="flex flex-col items-center justify-center gap-1 w-20 shrink-0 max-sm:w-full">
                    {/* Legacy matches predate series recording and have no
                        tally worth showing - they render exactly as before. */}
                    {match.games ? (
                      <>
                        <div className="font-bold text-lg tabular-nums text-text">
                          {match.team_a_games} - {match.team_b_games}
                        </div>
                        <div className="text-[0.7rem] font-semibold text-text-light">
                          {t("matchHistory.vs")}
                        </div>
                        {match.games.some(
                          (g) => g.a !== null && g.b !== null,
                        ) && (
                          <ol className="list-none p-0 m-0 mt-1 flex flex-col gap-0.5 text-[0.7rem] tabular-nums text-text-light">
                            {match.games.map((game, gameIndex) =>
                              game.a === null || game.b === null ? null : (
                                <li key={gameIndex}>
                                  {game.a} - {game.b}
                                </li>
                              ),
                ...
                <div className="flex justify-between items-center mt-4 pt-4 border-t border-border-light">
                  <div className="text-[0.85rem] text-text-light text-right">
                    {new Date(match.created_at).toLocaleString(DATE_LOCALE, {
                      day: "2-digit",
                      month: "2-digit",
                      year: "numeric",
                      hour: "2-digit",
                  ...
                  {season && (
                    <div className="match-season-badge">S{season.number}</div>
                  )}
