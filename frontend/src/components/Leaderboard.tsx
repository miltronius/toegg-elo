...
const PALETTE = ["#F97316","#FACC15","#34D399","#60A5FA","#F472B6","#A78BFA","#FB923C","#4ADE80"];
...
function buildSnapshots(players: Player[], history: EloHistory[]): Snapshot[] {
  ...
  for (const h of sorted) {
    if (!byMatch[h.match_id]) { byMatch[h.match_id] = []; matchOrder.push(h.match_id); }
    byMatch[h.match_id].push(h);
  }
  const currentElo: Record<string, number> = {};
  const firstSeen: Record<string, boolean> = {};
  for (const h of sorted) {
    if (!firstSeen[h.player_id]) { currentElo[h.player_id] = h.elo_before; firstSeen[h.player_id] = true; }
  }
  players.forEach(p => { if (currentElo[p.id] === undefined) currentElo[p.id] = p.current_elo; });
  ...
  matchOrder.forEach((matchId, matchIdx) => {
    ...
    ranked.forEach(({ id, elo }, i) =>
      snapshots.push({ player_id: id, elo, rank: i + 1, match_index: matchIdx })
    );
  });
  return snapshots;
}
...
function buildPath(
  ...
  const pts = matchIndices
    .map(m => snapshots.find(s => s.player_id === playerId && s.match_index === m))
    .filter(Boolean) as Snapshot[];
  if (pts.length < 2) return "";
  return pts.map((pt, i) => {
    const x = xScale(pt.match_index), y = yScale(pt.rank);
    if (i === 0) return `M ${x} ${y}`;
    const prev = pts[i - 1];
...
export function Leaderboard({ players, history, onPlayerClick }: LeaderboardProps) {
  const [view, setView]       = useState<"table" | "bump">("table");
  const [hovered, setHovered] = useState<string | null>(null);
  const [tooltip, setTooltip] = useState<{ x: number; y: number; player: Player; snap: Snapshot } | null>(null);
  ...
  const matchIndices  = [...new Set(snapshots.map(s => s.match_index))].sort((a, b) => a - b);
  ...
  const chartH = Math.max(isMobile ? 220 : 280, nPlayers * (isMobile ? 44 : 56));
  ...
  if (players.length === 0) {
    return (
      <div className="card">
        <h2>Leaderboard</h2>
        <p className="empty-state">No players yet. Create a player to get started!</p>
      </div>
    );
  }

  return (
    <div className={`card leaderboard-card${view === "bump" ? " lb-expanded" : ""}`}>
        ...
        <table className="leaderboard-table">
          ...
          <tbody>
            {sortedPlayers.map((player, index) => {
              ...
              return (
                <tr key={player.id} onClick={() => onPlayerClick(player)} className="clickable-row">
      ...
      {view === "bump" && (
        <div className="bump-chart-wrap">
            ...
            {sortedPlayers.map((p, i) => (
              <button
                ...
                style={{
                  ...
                  borderColor: hovered === p.id ? playerColor(i) : undefined,
                }}
                onMouseEnter={() => setHovered(p.id)}
                onMouseLeave={() => setHovered(null)}
                onClick={() => onPlayerClick(p)}
              >
                <span className="bump-legend-dot" style={{ background: playerColor(i) }} />
                {p.name}
              </button>
            ))}
          </div>
              ...
              >
                ...
                {Array.from({ length: nPlayers }, (_, i) => (
                  <g key={i}>
                    <line
                      ...
                      y1={yScale(i + 1)} y2={yScale(i + 1)}
                      stroke="var(--bump-grid)" strokeWidth={1}
                    />
                    <text
                      x={PAD.left - 6} y={yScale(i + 1)}
                ...
                {matchIndices
                  .filter((_, i) => {
                    ...
                    return i % Math.max(1, Math.ceil(matchIndices.length / maxLabels)) === 0
                      || i === matchIndices.length - 1;
                  })
                  .map(m => (
                    <text key={m} x={xScale(m)} y={chartH - 8}
                      className="bump-axis-label" textAnchor="middle">
                      M{m + 1}
                    </text>
                  ))}
                ...
                {sortedPlayers.map((player, i) => {
                  ...
                  const d      = buildPath(player.id, matchIndices, snapshots, xScale, yScale);
                  if (!d) return null;
                  return (
                    <path key={player.id} d={d}
                      fill="none" stroke={color}
                      strokeWidth={active ? 3.5 : isMobile ? 1.5 : 2}
                      strokeOpacity={dimmed ? 0.07 : active ? 1 : 0.5}
                      strokeLinecap="round"
                      style={{ transition: "stroke-opacity 0.2s, stroke-width 0.15s", cursor: "pointer" }}
                ...
                {sortedPlayers.map((player, i) => {
                  ...
                  return matchIndices.map(m => {
                    const snap = snapshots.find(s => s.player_id === player.id && s.match_index === m);
                    if (!snap) return null;
                    const baseR = isMobile ? 2.5 : 3.5;
                    return (
                      <circle key={`${player.id}-${m}`}
                        cx={xScale(snap.match_index)} cy={yScale(snap.rank)}
                        r={active ? baseR + 2 : baseR}
                        fill={color}
                        fillOpacity={dimmed ? 0.06 : active ? 1 : 0.65}
                        style={{ cursor: "pointer", transition: "r 0.15s, fill-opacity 0.2s" }}
                        onMouseEnter={() => { setHovered(player.id); setTooltip({ x: xScale(snap.match_index), y: yScale(snap.rank), player, snap }); }}
                        onMouseLeave={() => { setHovered(null); setTooltip(null); }}
                      />
                    );
                  });
                ...
                {!isMobile && sortedPlayers.map((player, i) => {
                  const last = matchIndices.at(-1);
                  const snap = snapshots.find(s => s.player_id === player.id && s.match_index === last);
                  if (!snap) return null;
                  const color  = playerColor(i);
                  const dimmed = hovered && hovered !== player.id;
                  return (
                    <text key={player.id}
                      x={svgWidth - PAD.right + 8} y={yScale(snap.rank)}
                ...
                {tooltip && (() => {
                  const pi     = sortedPlayers.findIndex(p => p.id === tooltip.player.id);
                  const color  = playerColor(pi);
                  const tipW   = 148, tipH = 50;
                  const tx     = tooltip.x + 12 + tipW > svgWidth ? tooltip.x - tipW - 12 : tooltip.x + 12;
                  const ty     = tooltip.y - tipH - 8 < 0 ? tooltip.y + 8 : tooltip.y - tipH - 8;
                  return (
                    <g style={{ pointerEvents: "none" }}>
                      <rect x={tx} y={ty} width={tipW} height={tipH} rx={7}
                        fill="var(--tooltip-bg)" stroke={color} strokeWidth={1} strokeOpacity={0.6}
                        style={{ filter: "drop-shadow(0 2px 6px rgba(0,0,0,0.12))" }}
                      />
                      <text x={tx + 10} y={ty + 17} fill={color} fontSize={12} fontWeight={700}>
                        {tooltip.player.name}
                      </text>
                      <text x={tx + 10} y={ty + 34} fill="var(--tooltip-sub)" fontSize={11}>
                        ELO {tooltip.snap.elo} · Rank #{tooltip.snap.rank} · M{tooltip.snap.match_index + 1}
                      </text>
                    </g>
                  );
                })()}
          ...
          <p className="bump-hint">
            {isMobile ? "Tap a line to highlight" : "Hover a line or dot to highlight · click a name to view player"}
          </p>
        </div>
      )}
    </div>
