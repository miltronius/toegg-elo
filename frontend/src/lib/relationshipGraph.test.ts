...
const friendFor = (
  g: ReturnType<typeof computeRelationshipGraph>,
  a: string,
  b: string,
...
describe("computeRelationshipGraph - edges produced per match", () => {
  ...
  it("records the winning pair as winning together and the losing pair as losing", () => {
    ...
    expect(friendFor(g, "p3", "p4")).toMatchObject({
      games: 1,
      wins: 0,
      losses: 1,
    });
...
describe("computeRelationshipGraph - foe records are stored from lo's side", () => {
  ...
  it("credits hi when the lo player loses", () => {
    ...
    expect(foeFor(g, "a", "z")).toMatchObject({
      games: 1,
      loWins: 0,
      hiWins: 1,
      loShare: 0,
  ...
  it("splits an even rivalry down the middle", () => {
    const g = computeRelationshipGraph(
      "s1",
      [makeMatch({ winning_team: "A" }), makeMatch({ winning_team: "B" })],
      PLAYERS,
    );
...
describe("computeRelationshipGraph - season scoping", () => {
  ...
  it("counts only the requested season", () => {
    expect(
      friendFor(computeRelationshipGraph("s1", matches, PLAYERS), "p1", "p2")!
        .games,
    ).toBe(1);
    expect(
      friendFor(computeRelationshipGraph("s2", matches, PLAYERS), "p1", "p2")!
        .games,
    ).toBe(2);
  });
  ...
  it("counts every season when seasonId is null", () => {
    expect(
      friendFor(computeRelationshipGraph(null, matches, PLAYERS), "p1", "p2")!
        .games,
    ).toBe(3);
  });
});
...
describe("computeRelationshipGraph - topEdgesPerPlayer and nodes", () => {
  ...
  it("thins a fully-connected league", () => {
    ...
    for (let i = 0; i < 6; i++) {
      for (let j = i + 1; j < 6; j++) {
        ...
        ms.push(
          makeMatch({
            team_a_player_1_id: six[i].id,
            team_a_player_2_id: six[rest[0]].id,
            team_b_player_1_id: six[j].id,
            team_b_player_2_id: six[rest[1]].id,
          }),
        );
      }
    }
  ...
  it("flags an unrequited favourite as one-way", () => {
    ...
    const matches = [
      makeMatch({
        team_a_player_1_id: "p3",
        team_a_player_2_id: "p4",
        team_b_player_1_id: "p5",
        team_b_player_2_id: "p6",
      }),
      makeMatch({
        team_a_player_1_id: "p3",
        team_a_player_2_id: "p4",
  ...
  it("drops edges pointing at a player who is no longer in the roster", () => {
    ...
    expect(g.nodes.map((n) => n.id)).toEqual(
      expect.not.arrayContaining(["p4"]),
    );
    expect(
      [...g.friends, ...g.foes].some((e) => e.lo === "p4" || e.hi === "p4"),
    ).toBe(false);
  ...
  it("carries the player display name and ELO onto the node", () => {
    const g = computeRelationshipGraph(
      "s1",
      [makeMatch()],
      [makePlayer("p1", "Ann", 1620), ...PLAYERS.slice(1)],
    );
...
describe("assignColorSlots", () => {
  ...
  it("is deterministic", () => {
    ...
    expect([...assignColorSlots(ids, edges, 6)]).toEqual([
      ...assignColorSlots(ids, edges, 6),
    ]);
  });
});
